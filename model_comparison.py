...
def compare_mag_models():
    ...
    mag_data = [ ['Long Short-Term Memory Network', mag_mse_ls], [mag_reg_key, mag_mse_re]]
    ...
    st.plotly_chart(fig)
    mag_comp_dict = {'Long Short-Term Memory Network': mag_mse_ls,
...
def compare_dep_models():
    ...
    data = [ ['Long Short-Term Memory Network', dep_mse_ls], [dep_reg_key, dep_mse_re]]
    ...
    fig = px.line(dep_comp_df, x="Algorithm", y="Mean Squared Error", title='Comparison of Algorithms',
                  template="plotly_dark")
    st.plotly_chart(fig)
    dep_comp_dict = {'Long Short-Term Memory Network': dep_mse_ls,
